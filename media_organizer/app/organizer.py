...
def organize_files(files, dest_folder, operation='copy', dry_run=False, tag_order=None, duplicate_mode='overwrite', use_earliest=False):
    ...
    for file_path in files:
        ...
        if not dt:
            # Move/copy to unsorted folder if date can't be parsed
            unsorted_dir = dest_folder / "unsorted"
            if not dry_run:
                ensure_dir(unsorted_dir)
            dest_path = unsorted_dir / Path(file_path).name
            dest_path = _get_nonconflicting_path(dest_path, duplicate_mode)
            try:
                if dry_run:
                    logger.info(f"[DRY RUN] Would {operation} {file_path} -> {dest_path} (unsorted)")
                    continue
                if operation == 'move':
                    shutil.move(str(file_path), str(dest_path))
                    logger.info(f"Moved {file_path} -> {dest_path} (unsorted)")
                else:
                    shutil.copy2(str(file_path), str(dest_path))
                    logger.info(f"Copied {file_path} -> {dest_path} (unsorted)")
            except Exception as e:
                logger.error(f"Failed to {operation} {file_path} to unsorted: {e}")
            continue
        year = str(dt.year)
