...
`media_organizer/agent.md`, `media_organizer/prompt.md`, and `media_organizer/MASTER_COPILOT_PROMPT.md` are earlier AI-collaboration prompts from this project's history — `prompt.md` in particular is a raw pasted chat transcript from the app's original design phase (predates the current file layout and the decision to drop Docker). All three describe an earlier, since-superseded architecture and workflow (a different module layout, Docker-based deployment, `overwrite`/`preserve` duplicate-handling modes that no longer exist). This file (`CLAUDE.md`) is the current, maintained guidance doc — treat those three as history, not instructions to follow.
